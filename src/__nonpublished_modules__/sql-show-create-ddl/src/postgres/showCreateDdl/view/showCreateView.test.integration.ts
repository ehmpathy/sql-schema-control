...
describe('showCreateView', () => {
  ...
  it('should be possible to get create statement of view', async () => {
    await dbConnection.query({ sql: 'DROP VIEW IF EXISTS test_view_for_show_create_on;' });
    await dbConnection.query({
      sql: "CREATE VIEW test_view_for_show_create_on as SELECT 'hello' as first_words",
    });
    const result = await showCreateView({ dbConnection, schema: 'public', name: 'test_view_for_show_create_on' });

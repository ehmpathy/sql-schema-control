...
*/
export const getUncontrolledResources = async ({ connection, controlledResources }: {
  ...
  const uncontrolledResources = await liveResources.filter((liveResource) => {
    const controlledResourceExists = controlledResources.some(resource => resource.name === liveResource.name && resource.type === liveResource.type);

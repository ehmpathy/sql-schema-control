import { getResourceFromDatabase, ResourceDoesNotExistError } from './getResourceFromDatabase';
import { DatabaseConnection, DatabaseLanguage, ControlConfig, ResourceDefinition, ResourceType } from '../../../../types';
...
describe('getResourceFromDatabase', () => {
  ...
  describe('table', () => {
    ...
    it('should throw an error if table is not defined', async () => {
      ...
      try {
        await getResourceFromDatabase({ connection, resourceName: resource.name, resourceType: resource.type });
    ...
    it('should be able to get sql of an existing table', async () => {
      ...
      // get status
      const liveResource = await getResourceFromDatabase({ connection, resourceName: resource.name, resourceType: resource.type });
...
END;
    ...
    it('should throw an error if function is not defined', async () => {
      ...
      try {
        await await getResourceFromDatabase({ connection, resourceName: resource.name, resourceType: resource.type });
        throw new Error('should not reach here');
      } catch (error) {
        expect(error.constructor).toEqual(ResourceDoesNotExistError);
      }

...
export const getRequiredActionForResource = ({ definition }: { definition: ResourceDefinition }) => {
  ...
  if (definition.status === ResourceDefinitionStatus.NOT_CONTROLED) return RequiredAction.MANUAL_PULL;
  if (definition.status === ResourceDefinitionStatus.OUT_OF_SYNC) {
    if (definition.type === ResourceType.FUNCTION || definition.type === ResourceType.PROCEDURE) return RequiredAction.REAPPLY; // only sprocs and funcs can be dropped and recreated by reapplying
    return RequiredAction.MANUAL_MIGRATION; // if its any other resource, a migration is nessesary; TODO - support automated migrations / migration help
  }

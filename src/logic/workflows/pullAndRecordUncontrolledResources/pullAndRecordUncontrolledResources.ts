...
*/
export const pullAndRecordUncontrolledResources = async ({ configPath, targetDir }: { configPath: string, targetDir: string }) => {
  ...
  const uncontrolledResources = plans.map(plan => plan.definition).filter(def => def.status === ResourceDefinitionStatus.NOT_CONTROLED) as ResourceDefinition[];

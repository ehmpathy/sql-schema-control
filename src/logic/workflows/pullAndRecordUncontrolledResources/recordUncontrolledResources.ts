...
export const recordUncontrolledResources = async ({ targetDir, uncontrolledResources }: { targetDir: string, uncontrolledResources: ResourceDefinition[] }) => {
  console.log(`pulling uncontrolled resource definitions into ${targetDir}`);
  await Promise.all(uncontrolledResources.map(async (resource) => {

...
const getDifferenceForResourceDefinitionMock =
...
describe('getResourceStatus', () => {
  ...
  it('should use getResourceCreateStatement accurately', async () => {
    getDifferenceForResourceDefinitionMock.mockReturnValueOnce({
      sql: '__SQL__',
    });
    await getStatusForResourceDefinition({
  ...
  it('should return status = NOT_APPLIED if getResourceCreateStatement throws ResourceDoesNotExistError', async () => {
    ...
    const resourceStatus = await getStatusForResourceDefinition({
      connection: '__CONNECTION__' as any,
      resource,
    });
  ...
  it('should return status = OUT_OF_SYNC if diff exists', async () => {
    getDifferenceForResourceDefinitionMock.mockResolvedValueOnce(
      '__DIFFERENCE__',
    );
    const resourceStatus = await getStatusForResourceDefinition({
      connection: '__CONNECTION__' as any,

...
export const provisionChangeLogTable = async ({ connection }: { connection: DatabaseConnection }) => {
  ...
  const createTableSql = await readFileAsync({ filePath: `${__dirname}/../../../../schemas/tables/schema_control_change_log.mysql.sql` }); // TODO: make independent of language (mysql currently)

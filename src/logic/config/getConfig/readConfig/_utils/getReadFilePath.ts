...
export const getReadFilePath = ({
  ...
  if (!startFromRootDirectory && !startFromAParentDirectory) {
    throw new Error(
      `all paths must be relative, '${relativePath}' is not; relative paths must start with ./ or ../`,
    );
  }

  // strip ./ from relative path
  const strippedRelativePath = startFromRootDirectory
    ? relativePath.slice(2)
    : relativePath; // if starting from parent, no need to modify

...
const extractResourceTypeAndNameFromDDLMock =
  extractResourceTypeAndNameFromDDL as jest.Mock;
...
describe('hydrate change definition content', () => {
  ...
  it('should throw an error if the path is not defiend', async () => {
    try {
      await hydrateResourceDefinitionContent({
  ...
  it('should get the type and name from extractResourceTypeAndNameFromDDL', async () => {
    ...
    expect(
      extractResourceTypeAndNameFromDDLMock.mock.calls[0][0],
    ).toMatchObject({
  ...
  it('should return a ResourceDefinition object', async () => {
    ...
    const result = await hydrateResourceDefinitionContent({
      readRoot: '__READ_ROOT__',
      content: exampleContent,
    });

...
describe('flattenDefinitionsRecursive', () => {
  ...
  it('should return all definitions given to it, if all are DefinitionObjects', async () => {
    ...
    const returnedDefinitions = await flattenDefinitionsRecursive({
      definitions,
  ...
  it('should should attempt to read the contents of a each yml file, if a file path is defined as a definition', async () => {
    ...
    await flattenDefinitionsRecursive({
      definitions,
      readRoot: '__READ_ROOT__',
    });
    expect(readYmlFileMock.mock.calls.length).toEqual(3);
    expect(readYmlFileMock.mock.calls[0][0]).toMatchObject({
      filePath: `__READ_ROOT__/${definitions[1]}`,
    }); // check accuracy
    expect(readYmlFileMock.mock.calls[1][0]).toMatchObject({
      filePath: `__READ_ROOT__/${definitions[3]}`,
    }); // check accuracy
    expect(readYmlFileMock.mock.calls[2][0]).toMatchObject({
      filePath: `__READ_ROOT__/${definitions[4]}`,
    }); // check accuracy
  });
  ...
  describe('paths should be relative to the file they were defined in; derive relative root from flattened configs', () => {
    ...
    it('should use new readRoot for the recursion', async () => {
      ...
      expect(
        validateAndHydrateDefinitionsYmlContentsMock.mock.calls[1][0],
      ).toMatchObject({
